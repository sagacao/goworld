...
func detectGoWorldPath(progrom string) {
	searchPaths := getGoSearchPaths()
	showMsg("go search paths: %s", strings.Join(searchPaths, string(os.PathListSeparator)))
	if progrom == "components" {
		progrom = filepath.Join("github.com", "sagacao", "goworld")
	}
	for _, sp := range searchPaths {
		goworldPath := filepath.Join(sp, "src", progrom) //filepath.Join(sp, "src", "github.com", "sagacao", "goworld")
