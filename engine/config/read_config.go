...
type KVDBConfig struct {
	...
	Url        string // MongoDB
	DB         string // MongoDB
	Collection string // MongoDB & Redis Prefix
...
func Get() *GoWorldConfig {
	...
	if goWorldConfig == nil {
		...
		gwlog.Infof(">>> config <<< RANK type = %s", goWorldConfig.Rank.Type)
	}
...
// GetRank returns the Rank config
func GetRank() *RankConfig {
	return &Get().Rank
}
...
func readGoWorldConfig() *GoWorldConfig {
	...
	for _, sec := range iniFile.Sections() {
		...
		} else if secName == "rank" {
			// rank config
			readRankConfig(sec, &config.Rank)
...
func readKVDBConfig(sec *ini.Section, config *KVDBConfig) {
	...
	for _, key := range sec.Keys() {
		...
		if name == "type" {
			...
			config.Auth = key.MustString(config.Auth)
		} else if name == "driver" {
			config.Driver = key.MustString(config.Driver)
		} else if strings.HasPrefix(name, "start_nodes_") {
			config.StartNodes.Add(key.MustString(""))
...
func readRankConfig(sec *ini.Section, config *RankConfig) {
	config.StartNodes = common.StringSet{}
	for _, key := range sec.Keys() {
		name := strings.ToLower(key.Name())
		if name == "type" {
			...
			config.Prefix = key.MustString(config.Prefix)
		} else if name == "auth" {
			config.Auth = key.MustString(config.Auth)
		} else if strings.HasPrefix(name, "start_nodes_") {
			config.StartNodes.Add(key.MustString(""))
	...
	validateRankConfig(config)
}

func validateRankConfig(config *RankConfig) {
	if config.Type == "" {
		// rank not enabled, it's OK
	} else if config.Type == "redis" {
		if config.Url == "" {
			gwlog.Fatalf("invalid %s rank config:\n%s", config.Type, DumpPretty(config))
		}
		_, err := strconv.Atoi(config.DB) // make sure db is integer for redis
		if err != nil {
			gwlog.Panic(errors.Wrap(err, "redis db must be integer"))
		...
		if len(config.StartNodes) == 0 {
			gwlog.Fatalf("must have at least 1 start_nodes for [rank].redis_cluster")
		}
		for s := range config.StartNodes {
			if s == "" {
				gwlog.Fatalf("start_nodes must not be empty")
		...
		gwlog.Fatalf("unknown rank type: %s", config.Type)
	}
}
...
func readDebugConfig(sec *ini.Section, config *DebugConfig) {
	...
	for _, key := range sec.Keys() {
		...
		if name == "debug" {
			...
			config.Identifier = key.MustString("")
		} else {
			gwlog.Fatalf("section %s has unknown key: %s", sec.Name(), key.Name())
		}
	}
